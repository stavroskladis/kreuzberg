...
- **LLM intelligence** – VLM OCR (GPT-4o, Claude, Gemini), structured JSON extraction with schema constraints, and provider-hosted embeddings via 142+ LLM providers through liter-llm
- **OCR support** – Tesseract (all bindings, including Tesseract-WASM for browsers), PaddleOCR (all native bindings), EasyOCR (Python), VLM OCR (142+ vision model providers), extensible via plugin API
