...
// alef:hash:f4819e4c9c35f89a9a7873b8812a95f8ffd2265a2e7b7537adf6d517b9398baf
...
impl FormatMetadata {
    ...
    #[php(getter)]
    pub fn get__0(&self) -> Option<String> {
        self._0.clone()
    }
}
    ...
    fn from(val: EmbeddingModelType) -> Self {
        match val.type_tag.as_str() {
            ...
            "custom" => kreuzberg::EmbeddingModelType::Custom {
                model_id: val.model_id.map(Into::into).unwrap_or_default(),
                dimensions: val.dimensions.map(Into::into).unwrap_or_default(),
            },
    ...
    fn from(val: NodeContent) -> Self {
        match val.node_type_tag.as_str() {
            ...
            "slide" => kreuzberg::NodeContent::Slide {
                number: val.number.map(Into::into).unwrap_or_default(),
            ...
            "citation" => kreuzberg::NodeContent::Citation {
                key: val.key.map(Into::into).unwrap_or_default(),
                text: val.text.map(Into::into).unwrap_or_default(),
            },
    ...
    fn from(val: AnnotationKind) -> Self {
        match val.annotation_type_tag.as_str() {
            ...
            "color" => kreuzberg::AnnotationKind::Color {
                value: val.value.map(Into::into).unwrap_or_default(),
            },
...
impl From<FormatMetadata> for kreuzberg::FormatMetadata {
    fn from(val: FormatMetadata) -> Self {
        match val.format_type_tag.as_str() {
            "pdf" => kreuzberg::FormatMetadata::Pdf {
                _0: val._0.map(Into::into).unwrap_or_default(),
            },
            "docx" => kreuzberg::FormatMetadata::Docx {
                _0: val._0.map(Into::into).unwrap_or_default(),
            },
            "excel" => kreuzberg::FormatMetadata::Excel {
                _0: val._0.map(Into::into).unwrap_or_default(),
            },
            "email" => kreuzberg::FormatMetadata::Email {
                _0: val._0.map(Into::into).unwrap_or_default(),
            },
            "pptx" => kreuzberg::FormatMetadata::Pptx {
                _0: val._0.map(Into::into).unwrap_or_default(),
            },
            "archive" => kreuzberg::FormatMetadata::Archive {
                _0: val._0.map(Into::into).unwrap_or_default(),
            },
            "image" => kreuzberg::FormatMetadata::Image {
                _0: val._0.map(Into::into).unwrap_or_default(),
            },
            "xml" => kreuzberg::FormatMetadata::Xml {
                _0: val._0.map(Into::into).unwrap_or_default(),
            },
            "text" => kreuzberg::FormatMetadata::Text {
                _0: val._0.map(Into::into).unwrap_or_default(),
            },
            "html" => kreuzberg::FormatMetadata::Html {
                _0: val._0.map(Into::into).unwrap_or_default(),
            },
            "ocr" => kreuzberg::FormatMetadata::Ocr {
                _0: val._0.map(Into::into).unwrap_or_default(),
            },
            "csv" => kreuzberg::FormatMetadata::Csv {
                _0: val._0.map(Into::into).unwrap_or_default(),
            },
            "bibtex" => kreuzberg::FormatMetadata::Bibtex {
                _0: val._0.map(Into::into).unwrap_or_default(),
            },
            "citation" => kreuzberg::FormatMetadata::Citation {
                _0: val._0.map(Into::into).unwrap_or_default(),
            },
            "fiction_book" => kreuzberg::FormatMetadata::FictionBook {
                _0: val._0.map(Into::into).unwrap_or_default(),
            },
            "dbf" => kreuzberg::FormatMetadata::Dbf {
                _0: val._0.map(Into::into).unwrap_or_default(),
            },
            "jats" => kreuzberg::FormatMetadata::Jats {
                _0: val._0.map(Into::into).unwrap_or_default(),
            },
            "epub" => kreuzberg::FormatMetadata::Epub {
                _0: val._0.map(Into::into).unwrap_or_default(),
            },
            "pst" => kreuzberg::FormatMetadata::Pst {
                _0: val._0.map(Into::into).unwrap_or_default(),
            },
            "code" => kreuzberg::FormatMetadata::Code {
                _0: val._0.map(Into::into).unwrap_or_default(),
            },
            _ => kreuzberg::FormatMetadata::Pdf { _0: Default::default() },
        }
    }
}
    ...
    fn from(val: OcrBoundingGeometry) -> Self {
        match val.type_tag.as_str() {
            "rectangle" => kreuzberg::OcrBoundingGeometry::Rectangle {
                left: val.left.map(Into::into).unwrap_or_default(),
                top: val.top.map(Into::into).unwrap_or_default(),
                width: val.width.map(Into::into).unwrap_or_default(),
                height: val.height.map(Into::into).unwrap_or_default(),
            },
            "quadrilateral" => kreuzberg::OcrBoundingGeometry::Quadrilateral {
                points: val.points.map(Into::into).unwrap_or_default(),
            },
